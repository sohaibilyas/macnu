...
// Macnu security patch: upstream 2.10.1 otherwise buffers both responses without a limit.
const UPDATE_MANIFEST_LIMIT: usize = 64 * 1024;
const UPDATE_DOWNLOAD_LIMIT: usize = 512 * 1024 * 1024;
const UPDATE_INITIAL_CAPACITY_LIMIT: usize = 8 * 1024 * 1024;

fn append_with_limit(buffer: &mut Vec<u8>, chunk: &[u8], limit: usize, label: &str) -> Result<()> {
    let next_length = buffer
        .len()
        .checked_add(chunk.len())
        .ok_or_else(|| Error::Network(format!("{label} exceeds the configured size limit")))?;
            ...
            match response {
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        if content_length
                            .is_some_and(|length| length > UPDATE_MANIFEST_LIMIT as u64)
                        {
                            return Err(Error::Network(
                                "Update manifest exceeds the configured size limit".to_string(),
                            ));
                        }
                        let initial_capacity = content_length
                            .and_then(|length| usize::try_from(length).ok())
                            .unwrap_or_default()
                            .min(UPDATE_MANIFEST_LIMIT);
                        let mut manifest = Vec::with_capacity(initial_capacity);
...
impl Update {
    ...
    pub async fn download<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        if content_length.is_some_and(|length| length > UPDATE_DOWNLOAD_LIMIT as u64) {
            return Err(Error::Network(
                "Update download exceeds the configured size limit".to_string(),
            ));
        }
        let initial_capacity = content_length
            .and_then(|length| usize::try_from(length).ok())
...
mod tests {
    ...
    #[test]
    fn macnu_response_limit_rejects_overflow_without_appending_it() {
        let mut buffer = vec![1, 2, 3];
        append_with_limit(&mut buffer, &[4, 5], 5, "test response").unwrap();
        assert_eq!(buffer, vec![1, 2, 3, 4, 5]);

        assert!(append_with_limit(&mut buffer, &[6], 5, "test response").is_err());
        assert_eq!(buffer, vec![1, 2, 3, 4, 5]);
    }
